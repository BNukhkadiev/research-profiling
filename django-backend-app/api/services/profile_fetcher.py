...
# Load and preprocess CORE rankings
...
class ProfileFetcher:
    ...
    def fetch_profile(self):
        ...
        if author and author.publications:
            ...
            logger.info(f"Fetching from DBLP for author PID: {self.pid}")

        # Fetch fresh data
        self.fetch_data()
        name, affiliations = self.get_name_and_affiliations()
        publications, coauthors_dict, coauthor_pids, topic_counts = self.parse_publications(name)

        result = self.compile_results(
            name, affiliations, self.pid, publications, coauthors_dict, coauthor_pids, topic_counts
        )
    ...
    def parse_publications(self, name):
        publications, coauthors_dict, coauthor_pids, topic_counts = [], defaultdict(int), {}, Counter()
        
        for pub in self.root.findall(".//r"):
            publ_info = pub.find("./*")
            if publ_info is not None:
                pub_data = self._parse_single_publication(publ_info, name, coauthors_dict, coauthor_pids, topic_counts)
    ...
    def compile_results(self, name, affiliations, pid, publications, coauthors_dict, coauthor_pids, topic_counts):
